...
class EncryptionTest extends TestCase
{
    ...
    public function database_secret_is_stored_encrypted()
    {
        ...
        {
            // Make sure that what's stored in the DB isnt the same as what's being used in the application
            $this->assertTrue($application_secret !== $database_secret);

            // Then when we decrypt the secret, it is the same.
            $this->assertTrue($application_secret === decrypt($database_secret));
        }
        else {
            $this->assertTrue($application_secret === $database_secret);
        }
